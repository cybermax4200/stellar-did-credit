...
- TypeScript SDK (`@stellar-did-credit/sdk`): added the `GovernanceClient`
  wrapper for proposal creation, weighted voting, execution, delayed weight
  application, proposal lookup, and proposal pagination. The exported
  `GovernanceProposal` type preserves Soroban `u64` and `i128` values as
  `bigint`, and `ProtocolConfig.governanceId` is optional for existing
  integrations (#500)
